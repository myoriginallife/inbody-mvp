...
import { getProfile, getRecords } from "@/lib/client-storage";
...
export function DashboardView() {
  ...
  if (!profile) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        <h1 className="text-2xl font-bold">기본 정보가 필요합니다</h1>
        <p className="mt-3 text-zinc-600">맞춤 추천을 위해 먼저 기본 정보를 입력해주세요.</p>
        <Link href="/onboarding" className="mt-6 inline-block rounded-full bg-emerald-600 px-6 py-3 font-medium text-white hover:bg-emerald-700">
          기본 정보 입력
        </Link>
      </div>
    );
  ...
  if (!record) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        ...
        <p className="mt-3 text-zinc-600">인바디 결과지 사진을 올리고 맞춤 추천을 받아보세요.</p>
        <Link href="/inbody/new" className="mt-6 inline-block rounded-full bg-emerald-600 px-6 py-3 font-medium text-white hover:bg-emerald-700">
          인바디 입력하기
        </Link>
      </div>
    );
  ...
  const { dietPlan, exercisePlan, rationales } = record;
      ...
      ? compareWithPrevious(
          {
            ...
            bodyFatMass: record.bodyFatMass ?? null,
            bmi: record.bmi,
          ...
          },
          {
            ...
            bodyFatMass: previousRecord.bodyFatMass ?? null,
            bmi: previousRecord.bmi,
            visceralFat: previousRecord.visceralFat ?? null,
    ...
    <div className="mx-auto max-w-5xl px-4 py-10">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">인바디 결과 기반 식단·운동 추천</p>
        </div>
        <Link href="/inbody/new" className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-700">
          새 인바디 입력
        </Link>
      </div>
      <section className="mt-8 rounded-2xl border border-emerald-100 bg-white p-6 shadow-sm">
        ...
        {record.imageDataUrl && (
          <div className="mt-6">
            ...
            <Image src={record.imageDataUrl} alt="인바디 결과지" width={400} height={300} className="max-h-64 rounded-lg border object-contain" unoptimized />
          </div>
        )}
      </section>
